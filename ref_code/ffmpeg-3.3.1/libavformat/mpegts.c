                               ...
                               const uint8_t *buf, int buf_size, int is_start)
{
    ...
    if (tss->section_h_size == -1 && tss->section_index >= 3) {
        ...
        fprintf(stderr, "seclen:[%x][%x][%d]\n", *(tss->section_buf), *(tss->section_buf + 1), len);
    }
    ...
    while (buf_size > 0) {
        ...
        case MPEGTS_HEADER:
            ...
            if (pes->data_index == PES_START_SIZE) {
                ...
                // chengang : PES packet starts here. following three bytes are packet_start_code_prefix
...
static int parse_pcr(int64_t *ppcr_high, int *ppcr_low, const uint8_t *packet)
{
    ...
    if (len == 0)
        return AVERROR_INVALIDDATA;
    flags = *p++;
    fprintf(stderr, "sdf[%x]sdf\n", flags);
    len--;
    if (!(flags & 0x10)) {
        fprintf(stderr, "00000023123sdf[%x]sdf\n", flags);
        return AVERROR_INVALIDDATA;
    }
    fprintf(stderr, "123123123sdf[%x]sdf\n", flags);
    if (len < 6)
        return AVERROR_INVALIDDATA;
    v          = AV_RB32(p);
